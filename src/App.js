...
//             <div className="status-title">Oil Level</div>
...
export default function Dashboard() {
  ...
  // Access vibration data directly from the database structure
  const vibrationData = monitoringData.vibration || {};
  
  // Get trend data (temperature change)
  const trendTemperature = monitoringData.trends?.temperature || -5.78;
      ...
      <div className="dashboard-container">
        ...
        <div className="status-grid">
          ...
          <div className="status-card vibration-card">
            <div className="status-title">Vibration Magnitude</div>
            <div className="status-value">
              {vibrationData.magnitude?.toFixed(4) || latest.vibration?.magnitude?.toFixed(4)}
              <span className="status-unit">g</span>
            </div>
            <div className="status-details">
              <div className="axis-value">X: {vibrationData.x?.toFixed(4) || latest.vibration?.x?.toFixed(4)}</div>
              <div className="axis-value">Y: {vibrationData.y?.toFixed(4) || latest.vibration?.y?.toFixed(4)}</div>
              <div className="axis-value">Z: {vibrationData.z?.toFixed(4) || latest.vibration?.z?.toFixed(4)}</div>
            </div>
          </div>
